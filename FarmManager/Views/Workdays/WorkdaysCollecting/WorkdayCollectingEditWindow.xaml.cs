...
public partial class WorkdayCollectingEditWindow : Window
{
    ...
    public WorkdayCollectingEditWindow(WorkdayCollecting workdayCollecting, ICollection<int> employeeIds)
    {
        InitializeComponent();
        employeeIds.Remove(workdayCollecting.EmployeeId);
        Loaded += async (_, __) => await ((WorkdayCollectingEditViewModel)DataContext).InitializeAsync(workdayCollecting, employeeIds);
